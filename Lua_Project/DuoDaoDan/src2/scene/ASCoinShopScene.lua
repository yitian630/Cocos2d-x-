-- 
-- ASCoinShopScene 购买金币场景
--

local ASCoinShopScene = class("ASCoinShopScene", function()
...
ASCoinShopScene.__index = ASCoinShopScene
...
-- @return：返回创建的对象
------------------------------------------------
function ASCoinShopScene.create()
    local scene = ASCoinShopScene.new()
    if nil ~= scene then
...
-------------------------------------------------
function ASCoinShopScene:init()
    -- 初始化背景颜色层
    local layer = cc.LayerColor:create(cc.c4b(125,125,125,255), size.width, size.height)
    self:addChild(layer)

    local labelTest = cc.Label:createWithTTF("CoinShopScene","fonts/Marker Felt.ttf",50)
